...
public class ComponentIdentityTest {
	...
	public void shouldCreateManyInstancesOfResource() throws Exception {
		...
		Set<Integer> otherHashCodes = Stream.generate(() -> restTemplate.getForEntity(url, Integer.class))
				.mapToInt(ResponseEntity::getBody)
				.limit(5)
				.boxed()
				.collect(toSet());

		assertThat(firstHashCode, isIn((otherHashCodes)));
		assertThat(otherHashCodes, hasSize(equalTo(1)));
	}
	...
	private String urlToGetResourceIdentity() {
		return BASE_URL + ComponentIdentity.PATH;
	}

	private String urlToGetNumberOfLoggers() {
		return BASE_URL + ComponentIdentity.PATH + "/numberOfLoggers";
	}
